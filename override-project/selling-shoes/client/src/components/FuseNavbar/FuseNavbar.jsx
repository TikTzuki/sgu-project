                ...
                {listMenu.map((menuItem) => {
                    if ((menuItem.hasOwnProperty('dropdownList'))) {
                        return (<div key={menuItem.name}>
                            <MenuItem component={Button} key={menuItem.name} disableRipple className={classes.buttonNav}
                                      drop-index={menuItem.name} variant="text" onClick={handleDropdown}>
                                {menuItem.name} {dropSwitcher === menuItem.name ?
                            ...
                            <Collapse in={dropSwitcher === menuItem.name ? true : false} timeout={300}>
                                <MenuList>
                                    {menuItem.dropdownList.map((dropDownItem) => {
                                        return (
                                            <MenuItem to={menuItem.to + dropDownItem.to} key={dropDownItem.name}
                                                      className={classes.nested} component={Link}
                                                      selected={menuItem.to + dropDownItem.to === pathname}>
                                                {dropDownItem.name}
                                            </MenuItem>
                                        );
                                    })}
                        ...
                        return (
                            <MenuItem key={menuItem.name} component={Link} to={menuItem.to}
                                      selected={menuItem.to === pathname}>
                                {menuItem.name}
                            </MenuItem>)
                    }
                })}
