<div class="page-wrapper">
    <?php 
        ...
        //Khoi tao query
        $query = "SELECT mabd,maloai,hinhanh,tenbd,gia,soluongtonkho,trangthai FROM bongden ";
        ...
        if(isset($_POST['search'])){
            ...
            if(isset($_POST['light_brand']) && $_POST['light_brand']!=""){
                $condition .= " AND thuonghieu='".$_POST['light_brand']."' ";
                $need_get .= "&light_brand='".$_POST['light_brand'];
            }
            $query .= $condition;
        }
    ...
    if(isset($_GET['page_num'])){
        ...
        if(isset($_GET['light_brand'])){
            $condition .= " AND thuonghieu='".$_GET['light_brand']."' ";
            $need_get .= "&light_brand='".$_GET['light_brand'];
        } 
        $query .= $condition;
    }
        ...
        <div class="row">
            <div class="row">
                ...
                <form action="?page=manager_product" method="POST">
                    ...
                    <input name="light_name" type="text" placeholder="Tên sản phẩm">
                    <label for=""> Khoảng giá </label>
                    <input name="light_min_price" type="text" placeholder="Thấp nhất" default="0">
                    <input name="light_max_price" type="text" placeholder="Cao nhất">
                    <label for=""> Thương hiệu </label>
                    <select name="light_brand" id="">
                        <option value=''></option>
                        <?php
                            $list_brand = $connect->select("SELECT tenthuonghieu as thuonghieu FROM thuonghieu,bongden WHERE bongden.mathuonghieu = thuonghieu.mathuonghieu GROUP BY tenthuonghieu");
                            while($row = $list_brand->fetch()){
                                echo "<option value='".$row['thuonghieu']."' >". $row['thuonghieu'] ."</option>";
                            }
                        ?>
                    </select>
                    ...
                    <?php 
                        while($row = $light_stmt->fetch()){
                            ...
                            <form action=\"./controller/change_product_perform.php?light_id=".$row['mabd']."\" method=\"POST\" enctype=\"multipart/form-data\">
                                ...
                                <td><span class=\"price-product\">". $row['gia']."</span></td>
                                <td><span class=\"quantity-product\">". $row['soluongtonkho']."</span></td>
                                <td>".$state."</td>
                                <td> 
                                    <ul> 
                                        <li> <input type=\"submit\" name=\"submit_manager_product\" value=\"save_change\"> </input></li>
                                        <li><a href=\"?page=change_product&light_id=".$row['mabd']."\"> Sửa </a></li>
                                        <li><a href=\"./controller/delete_product_perform.php?light_id=".$row['mabd']."\"> Xóa </a></li>
                                    </ul>
                                </td>
                            </form>
                            </tr>"
